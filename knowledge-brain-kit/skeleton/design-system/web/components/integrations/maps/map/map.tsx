...
const Map = forwardRef<MapRef, MapProps>(function Map(
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // React StrictMode (dev) monta y desmonta el efecto dos veces seguidas. Si
      // se llama a map.remove() mientras el estilo aún está cargando, maplibre-gl
      // 5.x ejecuta su `_load` sobre un mapa ya destruido (this.style === undefined)
      // y lanza "Cannot read properties of undefined (reading 'projection')".
      // Diferimos la retirada hasta que el estilo haya cargado (con un tope por si
      // la carga nunca completa, p. ej. navegación muy rápida sin red).
      if (map.loaded()) {
        try {
          map.remove();
        } catch {
        ...
        const fallback = setTimeout(safeRemove, 5000);
