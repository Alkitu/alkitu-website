      ...
      this.items.map(
        item =>
          new Promise<HTMLImageElement>(resolve => {
            ...
            img.onerror = () => {
              // Create a tiny transparent fallback image on error so it doesn't hang
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [], scale = 1.0, className }) => {
  ...
  return (
    <>
      <style>{`
        ...
        #igm-canvas:active { cursor: grabbing; }
        .igm-action-button {
            position: absolute;
            left: 50%;
            z-index: 10;
        ...
        .igm-face-title.active {
            opacity: 1;
            transform: translate(20%, -50%);
            pointer-events: auto;
            transition: 0.5s ease;
        }
        .igm-face-title.inactive {
            pointer-events: none;
            opacity: 0;
            transition: 0.1s ease;
        }
        .igm-face-description {
            user-select: none;
            position: absolute;
            max-width: 10ch;
            top: 50%;
        ...
        .igm-face-description.active {
            opacity: 1;
            transform: translate(-90%, -50%);
            pointer-events: auto;
            transition: 0.5s ease;
        }
        .igm-face-description.inactive {
            pointer-events: none;
            transform: translate(-60%, -50%);
            opacity: 0;
            transition: 0.1s ease;
        }
        .igm-action-button.active {
            bottom: 3.8em;
        ...
        @media (max-width: 1500px) {
            .igm-face-title,
            .igm-face-description { display: none; }
        }
      `}</style>
      <div style={{ position: 'relative', width: '100%', height: '100%' }}>
        <canvas id="igm-canvas" ref={canvasRef} />
        {activeItem && (
          <>
            <h2 className={`igm-face-title ${isMoving ? 'inactive' : 'active'}`}>{activeItem.title}</h2>
            <p className={`igm-face-description ${isMoving ? 'inactive' : 'active'}`}>{activeItem.description}</p>
            <div onClick={handleButtonClick} className={`igm-action-button ${isMoving ? 'inactive' : 'active'}`}>
              <p className="igm-action-button-icon">&#x2197;</p>
            </div>
          </>
        )}
      </div>
