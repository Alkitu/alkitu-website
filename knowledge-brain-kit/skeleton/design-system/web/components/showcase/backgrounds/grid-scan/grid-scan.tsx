...
export const GridScan: React.FC<GridScanProps> = ({
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      const tiltSm = smoothDampFloat(
        tiltCurrent.current, tiltTarget.current, { v: tiltVel.current }, smoothTime, maxSpeed, dt
      );
      tiltCurrent.current = tiltSm.value;
      tiltVel.current = tiltSm.v;

      const yawSm = smoothDampFloat(
        yawCurrent.current, yawTarget.current, { v: yawVel.current }, smoothTime, maxSpeed, dt
      );
  ...
  }, [sensitivity, lineThickness, linesColor, scanColor, scanOpacity, gridScale, lineStyle,
    lineJitter, scanDirection, enablePost]);
  ...
  useEffect(() => {
    if (!enableWebcam || !enableWebcam) return;
    let canceled = false;
    const load = async () => {
      try {
  ...
  return (
    <div ref={containerRef} className={cn("gridscan", className)} style={style}>
      {showPreview && (
...
function smoothDampVec2(
  current: THREE.Vector2, target: THREE.Vector2, currentVelocity: THREE.Vector2,
...
function smoothDampFloat(
  current: number, target: number, velRef: { v: number },
  smoothTime: number, maxSpeed: number, deltaTime: number
