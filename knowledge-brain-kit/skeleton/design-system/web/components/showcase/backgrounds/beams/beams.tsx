...
function extendMaterial(
  ...
  const defaults = new BaseMaterial((cfg.material || {}) as THREE.MeshStandardMaterialParameters) as THREE.MeshStandardMaterial;
...
const MergedPlanes = forwardRef<THREE.Mesh, MergedPlanesProps>(({ material, width, count, height }, ref) => {
  ...
  useFrame((_, delta) => {
    if (mesh.current?.material && 'uniforms' in (mesh.current.material as THREE.ShaderMaterial)) {
