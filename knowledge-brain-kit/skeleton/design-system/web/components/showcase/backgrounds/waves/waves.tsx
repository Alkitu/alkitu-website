...
export default function Waves({
    ...
    const linesRef = useRef<{ x: number; y: number; wave: { x: number; y: number }; cursor: { x: number; y: number; vx: number; vy: number } }[][]>([]);
    ...
    useEffect(() => {
        ...
        function moved(point: typeof linesRef.current[0][0], withCursor = true) {
            return {
    ...
    return (
        <div ref={containerRef} className={`waves ${className}`}
