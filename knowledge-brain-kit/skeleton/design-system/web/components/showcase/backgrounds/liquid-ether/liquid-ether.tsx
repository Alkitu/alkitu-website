...
export default function LiquidEther({
    ...
    useEffect(() => {
        ...
        class MouseClass {
            ...
            timer: number | null = null; container: HTMLElement | null = null;
        ...
        class ShaderPass {
            ...
            constructor(props: Record<string, unknown>) { this.props = props || {}; this.uniforms = (this.props.material as Record<string, unknown>)?.uniforms as Uniforms | undefined; }
            init() {
        ...
        class Advection extends ShaderPass {
            ...
            constructor(simProps: Record<string, unknown>) {
                super({ material: { vertexShader: face_vert, fragmentShader: advection_frag, uniforms: { boundarySpace: { value: simProps.cellScale }, px: { value: simProps.cellScale }, fboSize: { value: simProps.fboSize }, velocity: { value: (simProps.src as THREE.WebGLRenderTarget).texture }, dt: { value: simProps.dt }, isBFECC: { value: true } } }, output: simProps.dst });
            ...
            update(args?: { dt?: number; isBounce?: boolean; BFECC?: boolean }) {
                ...
                if (args?.dt != null) this.uniforms.dt.value = args.dt;
                if (args?.isBounce != null) this.line.visible = args.isBounce;
                if (args?.BFECC != null) this.uniforms.isBFECC.value = args.BFECC;
                super.update();
            }
        }
        ...
        class ExternalForce extends ShaderPass {
            mouse!: THREE.Mesh;
            constructor(simProps: Record<string, unknown>) { super({ output: simProps.dst }); this.initForce(simProps); }
            ...
            update(props?: Record<string, unknown>) {
                const p = props || {};
                const forceX = (Mouse.diff.x / 2) * ((p.mouse_force as number) || 0);
                const forceY = (Mouse.diff.y / 2) * ((p.mouse_force as number) || 0);
                const cellScale = (p.cellScale as THREE.Vector2) || { x: 1, y: 1 };
                const cursorSize = (p.cursor_size as number) || 0;
        ...
        class Viscous extends ShaderPass {
            ...
            update(args?: { viscous?: number; iterations?: number; dt?: number }) {
                ...
                if (args?.viscous != null) this.uniforms.v.value = args.viscous;
                const iter = args?.iterations ?? 0;
                for (let i = 0; i < iter; i++) {
                    const fbo_in = (i % 2 === 0 ? this.props.output0 : this.props.output1) as THREE.WebGLRenderTarget;
        ...
        class Poisson extends ShaderPass {
            ...
            update(args?: { iterations?: number }) {
                ...
                const iter = args?.iterations ?? 0;
                for (let i = 0; i < iter; i++) {
                    const p_in = (i % 2 === 0 ? this.props.output0 : this.props.output1) as THREE.WebGLRenderTarget;
        ...
        class Simulation {
            ...
            update() {
                ...
                let vel: THREE.WebGLRenderTarget | undefined = this.fbos.vel_1 as THREE.WebGLRenderTarget;
        ...
        class WebGLManager implements LiquidEtherWebGL {
            props: Record<string, unknown>; output!: Output; autoDriver?: AutoDriver;
            ...
            constructor(props: Record<string, unknown>) {
                this.props = props;
                Common.init(props.$wrapper as HTMLElement); Mouse.init(props.$wrapper as HTMLElement);
            ...
            init() { if (!Common.renderer) return; (this.props.$wrapper as HTMLElement).prepend(Common.renderer.domElement); this.output = new Output(); }
