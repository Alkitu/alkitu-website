 ...
 *
 * Manages the work-experience array: company, bilingual role title, location,
 * date range, bilingual bullet points (one per line), optional tech list, and
 * per-item privacy/ordering — following the same conventions as SkillsManager
 * and ProfileRolesManager.
 */
...
import type { ProfileExperience } from '@/lib/types/profiles';
...
export function ExperienceManager({ experience, onChange }: ExperienceManagerProps) {
  const sorted = [...experience].sort((a, b) => a.display_order - b.display_order);

  const handleAdd = () => {
    const maxOrder = experience.length > 0 ? Math.max(...experience.map((e) => e.display_order)) : -1;
    const newItem: ProfileExperience = {
      company: '',
      role: { en: '', es: '' },
      location: null,
      start_date: '',
  ...
  const handleUpdate = <K extends keyof ProfileExperience>(
    index: number,
    field: K,
      ...
      <div className="flex items-center justify-between">
        <label className="block text-sm font-medium text-foreground">
          ...
          <span className="ml-2 text-xs text-muted-foreground">
            ({sorted.length} {sorted.length === 1 ? 'puesto' : 'puestos'})
          </span>
        </label>
        <button
          type="button"
      ...
      {sorted.length === 0 ? (
        <div className="rounded-md border border-dashed border-border p-8 text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            No hay experiencia agregada. Haz clic en &quot;Agregar&quot; para comenzar.
          </p>
        </div>
      ) : (
        <div className="space-y-4">
          {sorted.map((item, index) => (
            <div key={index} className="rounded-md border border-border bg-background p-4 space-y-3">
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                <div>
                  <label className="block text-xs font-medium text-muted-foreground mb-1">Puesto (EN)</label>
                  <input
                    type="text"
                    value={item.role.en}
                    onChange={(e) => handleUpdate(index, 'role', { ...item.role, en: e.target.value })}
                    className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm text-foreground focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary"
                  />
                </div>
                <div>
                  <label className="block text-xs font-medium text-muted-foreground mb-1">Puesto (ES)</label>
                  <input
                    type="text"
                    value={item.role.es}
                    onChange={(e) => handleUpdate(index, 'role', { ...item.role, es: e.target.value })}
                    className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm text-foreground focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm text-foreground focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary disabled:opacity-50"
                  />
                </div>
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                <div>
                  <label className="block text-xs font-medium text-muted-foreground mb-1">
                    Bullets (EN) — uno por línea
                  </label>
                  <textarea
                    value={bulletsToText(item.bullets.en)}
                    onChange={(e) =>
                      handleUpdate(index, 'bullets', { ...item.bullets, en: textToBullets(e.target.value) })
                    }
                    rows={4}
                    className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm text-foreground focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary resize-none"
                  />
                </div>
                <div>
                  <label className="block text-xs font-medium text-muted-foreground mb-1">
                  ...
                  <textarea
                    value={bulletsToText(item.bullets.es)}
                    onChange={(e) =>
                      handleUpdate(index, 'bullets', { ...item.bullets, es: textToBullets(e.target.value) })
                    }
                    rows={4}
                    className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm text-foreground focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary resize-none"
                  />
              ...
              <div>
                <label className="block text-xs font-medium text-muted-foreground mb-1">
                  Tech (opcional, separado por comas)
                </label>
                <input
                  type="text"
                  ...
                  onChange={(e) =>
                    handleUpdate(
                      index,
                      'tech',
                      e.target.value ? e.target.value.split(',').map((t) => t.trim()).filter(Boolean) : null
                    )
                  }
                  placeholder="ej. NextJS, TailwindCSS, Prisma"
                  className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm text-foreground focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary"
                />
              </div>
