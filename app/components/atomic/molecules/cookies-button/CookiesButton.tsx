...
const DynamicModalCookies = dynamic(() => import('@/app/components/atomic/molecules/modal/modalCookies'), {
  ssr: false,
  loading: () => <p>Loading...</p>
})
export default function CookiesButton () {
  ...
  // No renderizar hasta que las traducciones estén disponibles
  if (!translations?.settings?.cookiesButton) {
    return null;
  }
        ...
        >
          ...
          >
            {show && (
              ...
              >
                {translations.settings.cookiesButton}
              </motion.p>
            )}
          </AnimatePresence>
          <div className="w-6 h-6" id="settingsButton" />
          <Script src="/js/lottie.js" strategy="beforeInteractive" />
